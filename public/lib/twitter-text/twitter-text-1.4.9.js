      ...
      if (index % 4 !== 0) {
        ...
        newText += chunk.replace(twttr.txt.regexen.autoLinkUsernamesOrLists, function(match, before, at, user, slashListname, offset, chunk) {
          ...
          if (slashListname && !options.suppressLists) {
            ...
            return stringSupplant("#{before}<a class=\"#{urlClass} #{listClass}\" href=\"#{listUrlBase}#{list}\"#{extraHtml}><span class=\"deemphasize\">#{at}</span>#{preChunk}#{chunk}#{postChunk}</a>", d);
          } else {
            if (after && after.match(twttr.txt.regexen.endScreenNameMatch)) {
              ...
              return stringSupplant("#{before}<a class=\"#{urlClass} #{usernameClass}\" #{dataScreenName}href=\"#{usernameUrlBase}intent/user?screen_name=#{chunk}\"#{extraHtml}><span class=\"deemphasize\">#{at}</span>#{preChunk}#{chunk}#{postChunk}</a>", d);
            }
          }
        });
      }
